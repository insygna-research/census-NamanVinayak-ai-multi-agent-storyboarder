...
Please analyze this and output your creative vision as JSON exactly as specified in your system instructions.`;
    ...
    console.log('Sending request to Director Agent RunPod endpoint...');
    ...
    if (result.id) {
      ...
      while (attempts < maxAttempts) {
        ...
        if (statusResult.status === 'COMPLETED') {
          ...
          if (statusResult.output?.[0]?.choices?.[0]?.message?.content) {
            const directorResponse = statusResult.output[0].choices[0].message.content;
          ...
          return NextResponse.json({
            error: 'Director Agent request failed',
            details: statusResult
          }, { status: 500 });
        }
      ...
      return NextResponse.json({
        error: 'Director Agent request timed out',
    ...
    if (result.status === 'COMPLETED' && result.output?.[0]?.choices?.[0]?.message?.content) {
      const directorResponse = result.output[0].choices[0].message.content;
      ...
      if (result.status === 'IN_PROGRESS' || result.status === 'IN_QUEUE') {
        return NextResponse.json({
          error: 'Director Agent request is still processing',
      ...
      return NextResponse.json({
        error: 'Unexpected response from Director Agent',
        status: result.status,
        details: result
      }, { status: 500 });
    }
